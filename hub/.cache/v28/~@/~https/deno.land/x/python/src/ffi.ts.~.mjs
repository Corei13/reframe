export default async (reframe_7dtf6y4yrvw) => {
    import.meta.path = "/~https/deno.land/x/python/src/ffi.ts";
    const imports_kx1b1yub8e = await reframe_7dtf6y4yrvw.importMany("./symbols.ts", "./util.ts");
    let exports_r5soy6m40n = {};
    exports_r5soy6m40n[Symbol.toStringTag] = "Module";
    const { SYMBOLS: SYMBOLS } = imports_kx1b1yub8e["./symbols.ts"];
    const { postSetup: postSetup } = imports_kx1b1yub8e["./util.ts"];
    const searchPath = [];
...
`);
    ...
    exports_r5soy6m40n["py"] = py;
    return exports_r5soy6m40n;
};
