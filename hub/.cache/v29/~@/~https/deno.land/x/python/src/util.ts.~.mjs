export default async () => {
    ...
    ;
    let exports_4a8qq55yivp = {};
    exports_4a8qq55yivp[Symbol.toStringTag] = "Module";
    const encoder = new TextEncoder();
    exports_4a8qq55yivp["encoder"] = encoder;
    const decoder = new TextDecoder();
    exports_4a8qq55yivp["decoder"] = decoder;
    const libDlDef = {
        dlopen: {
    ...
    exports_4a8qq55yivp["postSetup"] = postSetup;
    function cstr(str) {
        const buf = new Uint8Array(str.length + 1);
        encoder.encodeInto(str, buf);
        return buf;
    }
    exports_4a8qq55yivp["cstr"] = cstr;
    const SliceItemRegExp = /^\s*(-?\d+)?\s*:\s*(-?\d+)?\s*(:\s*(-?\d+)?\s*)?$/;
    exports_4a8qq55yivp["SliceItemRegExp"] = SliceItemRegExp;
    return exports_4a8qq55yivp;
};
