export default async (reframe_7dbz2smiqh8) => {
    import.meta.path = "/~https/deno.land/x/python/src/ffi.ts";
    const imports_l3ifrai12i = await reframe_7dbz2smiqh8.importMany("./symbols.ts", "./util.ts");
    let exports_fwsk171fqhg = {};
    exports_fwsk171fqhg[Symbol.toStringTag] = "Module";
    const { SYMBOLS: SYMBOLS } = imports_l3ifrai12i["./symbols.ts"];
    const { postSetup: postSetup } = imports_l3ifrai12i["./util.ts"];
    const searchPath = [];
...
`);
    ...
    exports_fwsk171fqhg["py"] = py;
    return exports_fwsk171fqhg;
};
