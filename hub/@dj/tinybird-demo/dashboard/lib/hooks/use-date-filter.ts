...
import { useCallback, useEffect, useMemo, useState } from "npm:react@canary";
import { DateRangePickerValue } from "npm:@tremor/react@2.0.2";
import { DateFilter, dateFormat } from "../types/date-filter.ts";
...
export default function useDateFilter() {
  ...
  const lastDays: DateFilter = typeof lastDaysParam === "string" &&
  ...
  const { startDate, endDate } = useMemo(() => {
    ...
    const startDate = moment(today)
      ...
      .format(dateFormat);
    const endDate = lastDays === DateFilter.Yesterday
      ? moment(today)
  ...
  const onDateRangePickerValueChange = useCallback(
    (_value: DateRangePickerValue) => {
      ...
      const [startDate, endDate, value] = _value;
